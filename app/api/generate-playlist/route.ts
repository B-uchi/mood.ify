...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (body.seedArtists) {
      seedArtists =
        body.seedArtists.length > 0
          ? `seed_artists=${body.seedArtists.join("%2C")}&`
          : "";
    } else {
    ...
    if (body.seedTracks) {
      seedTracks =
        body.seedTracks.length > 0
          ? `seed_tracks=${body.seedTracks.join("%2C")}&`
          : "";
    }else {
    ...
    if (body.seedGenres) {
      seed_genres =
        body.seedGenres.length > 0
          ? `seed_genres=${body.seedGenres.join("%2C")}&`
          : "";
    }else {
